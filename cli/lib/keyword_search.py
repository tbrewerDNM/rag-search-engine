...
class InvertedIndex:
    ...
    def __get_avg_doc_length(self) -> float:
        ...
        return total / len(self.doc_lengths.keys())
    ...
    def bm25_search(self, query, limit):
        ...
        for token in tokens:
            for doc_id in self.get_documents(token):
                found_docs[doc_id] = self.docmap[doc_id]

        found_docs = found_docs.values()

        for doc in found_docs:
            scores[doc['id']] = 0

            for token in tokens:
                scores[doc['id']] += self.bm25(doc['id'], token)

        found_docs = sorted(found_docs, key=lambda doc: scores[doc['id']], reverse=True)

        if len(found_docs) > limit:
            found_docs = found_docs[0:limit]

        return [(doc, scores[doc['id']]) for doc in found_docs]
